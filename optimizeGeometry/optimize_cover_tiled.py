...
The Fourier parameterisation guarantees the surface stays in [0, global_max + tile_max]
...
# =============================================================================
# SURFACE PREVIEW  (diagnostic helper, not used in objective)
# =============================================================================
def print_tiled_stats(dv: np.ndarray, cfg: FEAConfig) -> None:
    """Print a summary of the two-level tiled surface. Called after optimisation."""
    BX_O  = 788.0
    Z_MIN = -1016.0
    xs = np.linspace(0, BX_O,  60, dtype=np.float64)
    zs = np.linspace(Z_MIN, -10, 60, dtype=np.float64)
    xx, zz = np.meshgrid(xs, zs)
    heights = evaluate_tiled_surface(dv, cfg, xx.ravel(), zz.ravel())
    n_g = cfg.n_global_x * cfg.n_global_z * 4
    n_t = cfg.n_tile_x   * cfg.n_tile_z   * 4
    global_coeffs = dv[:n_g]
    tile_coeffs   = dv[n_g:]
    print(f"  Tiled surface stats:")
    print(f"    Global layer: {cfg.n_global_x}x{cfg.n_global_z}x4 = {n_g} DVs  "
          f"(max {cfg.global_max} mm)")
    print(f"    Tile layer:   {cfg.n_tile_x}x{cfg.n_tile_z}x4 = {n_t} DVs  "
          f"(max {cfg.tile_max} mm)  @ {cfg.tile_x}x{cfg.tile_z} mm tiles")
    print(f"    Global coeff: [{global_coeffs.min():.3f}, {global_coeffs.max():.3f}] mm")
    print(f"    Tile coeff:   [{tile_coeffs.min():.3f}, {tile_coeffs.max():.3f}] mm")
    print(f"    Combined:     [{heights.min():.2f}, {heights.max():.2f}] mm  "
          f"(max {cfg.global_max + cfg.tile_max:.2f} mm)")
...
def objective(dv: np.ndarray,
    ...
    [0, global_max + tile_max] — no structural penalty term is needed here.
...
def _load_run_args(output_dir):
    """
    Parse run_args.txt from output_dir and return a dict of key -> value
    for all arguments stored there.  Returns empty dict if file not found.
    """
    path = os.path.join(output_dir, "run_args.txt")
    if not os.path.exists(path):
        return {}
    # Types for each argument we care about
    int_args   = {"n_global_x", "n_global_z", "n_tile_x", "n_tile_z",
                  "max_evals", "n_init", "seed", "mcmc_warmup", "mcmc_samples",
                  "acqf_restarts", "acqf_raw_samples", "max_gp_obs", "tet_timeout"}
...
def save_run_args(args):
    ...
    arg_lines = ["python optimize_cover_tiled.py \\"]
...
def optimize(args):
    ...
    if not args.resume:
        ...
        print(f"  (Resuming — run_args.txt not overwritten)")
    ...
    lb_arr = np.array([-global_bound] * n_g + [-tile_bound] * n_t)
    ub_arr = np.array([ global_bound] * n_g + [ tile_bound] * n_t)
    bounds_t = torch.tensor(np.stack([lb_arr, ub_arr]), dtype=dtype, device=device)

    print("=" * 60)
    print("SAASBO Cover Geometry Optimizer (Tiled)")
    print("=" * 60)
    ...
    print(f"  Tile size:         {cfg.tile_x}x{cfg.tile_z} mm")
    print(f"  Height budget:     global {cfg.global_max} mm + tile {cfg.tile_max} mm "
          f"= {cfg.global_max + cfg.tile_max} mm total")
    ...
    if args.resume:
        ...
        X_all   = unnormalize(X_unit, bounds_t)   # maps [0,1]^n → per-DV bounds
    ...
    while total_evals < args.max_evals:
        ...
        if n_obs > args.max_gp_obs:
            # 75% best by objective, 25% most recent.
            # Weighting heavily toward best keeps the GP focused on the known
            # good region as the run grows, preventing the acquisition function
            # from drifting into unexplored bad regions late in a run.
            n_best   = int(args.max_gp_obs * 0.75)
            n_recent = args.max_gp_obs - n_best
            best_idx   = torch.argsort(Y.squeeze())[:n_best]
            recent_idx = torch.arange(
                max(0, n_obs - n_recent), n_obs,
                device=device
            )
            ...
            print(f"  GP training on {keep.shape[0]}/{n_obs} observations "
                  f"({n_best} best + {n_recent} recent, capped at {args.max_gp_obs})")
        else:
            X_fit = X
            Y_fit = Y
...
def main():
    ...
    # Pre-parse --output-dir and --resume so we can load run_args.txt before
    # building the full parser — this makes resume require only:
    #   python optimize_cover.py --resume --output-dir <dir>
    # All other arguments are loaded automatically from run_args.txt.
    ...
    if _pre_args.resume:
        ...
        if _run_args:
            ...
            print(f"  WARNING: --resume specified but no run_args.txt found in "
                  f"{_pre_args.output_dir}. Using command-line args / defaults.")

    def _d(key, default):
        """Return value from run_args.txt if resuming, else the given default."""
        return _run_args.get(key, default)

    p = argparse.ArgumentParser(
        description="SAASBO optimization of cover geometry (tiled parameterisation).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # ── Optimizer ────────────────────────────────────────────────────────────
    p.add_argument("--max-evals",    type=int,   default=_d("max_evals", 300),
    ...
    p.add_argument("--max-gp-obs",    type=int,   default=_d("max_gp_obs", 300),
