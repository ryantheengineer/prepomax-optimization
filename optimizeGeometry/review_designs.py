...
def main():
    ...
    p.add_argument("--output-dir",  default="opt_results",
                   help="Directory containing bo_checkpoint.pkl")
    ...
    # ── Fourier / grid (must match the run that produced the checkpoint) ──────
    ...
    p.add_argument("--grid-spacing",type=float, default=cfg_default.grid_spacing,
                   help="Grid spacing (mm) used during the optimization run. "
                        "Must match the run value so the Fourier domain lengths "
                        "are computed correctly.")
    p.add_argument("--vis-spacing", type=float, default=None,
                   help="Grid spacing (mm) for visualisation rendering. "
                        "Defaults to --grid-spacing (exact match to optimization). "
                        "Set finer (e.g. 5) for smoother plots.")
